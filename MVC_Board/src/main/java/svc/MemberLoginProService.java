...
public class MemberLoginProService {
	...
	public boolean isAuthenticatedUser(MemberBean member) {
		// 1. 인증 여부 판별 결과를 저장할 변수 선언(boolean 타입 isAuthenticatedUser)
		...
		// 5. MemberDAO 객체의 selectAuthStatus() 메서드를 호출하여 인증 여부 판별 작업 요청
		// => 파라미터 : MemberBean 객체   리턴타입 : boolean(isAuthenticatedUser)
		isAuthenticatedUser = dao.selectAuthStatus(member);
		
		// 6. Connection 객체 반환하기(공통)
		JdbcUtil.close(con);
		
		// 7. 로그인 판별 결과 리턴
		return isAuthenticatedUser; // MemberLoginProAction 으로 리턴
	}
