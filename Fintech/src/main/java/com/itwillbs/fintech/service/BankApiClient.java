...
import com.itwillbs.fintech.generator.BankValueGenerator;
...
//		jo.put("tran_amt", 1000); // tran_amt(거래금액 = 출금액)
		...
		jo.put("req_client_name", "xxx"); // req_client_name(요청고객성명)
...
//		jo.put("recv_client_account_num", "99999999999999"); // recv_client_account_num(최종수취 고객 계좌번호(검증X))
	...
	public AccountDepositListResponseVO deposit(Map<String, String> map) {
		...
		req.put("print_content", "이연태가송금"); // dps_print_content(입금계좌 인자내역, 상대방 통장에 표시할 내역)
		req.put("tran_amt", map.get("tran_amt")); // 입금금액
		req.put("req_client_name", "이연태"); // req_client_name(요청고객성명)
		...
		jo.put("wd_print_content", "7066출금"); // wd_print_content(출금계좌 인자내역, 내 통장에 표시할 내역)
