...
<%-- 	<h3>변경 후 현재 세션 유지시간 : <%=session.getMaxInactiveInterval() %>초</h3> --%>
	
	<%-- ===================================================================== --%>
	<%-- 세션 강제 초기화(= 세션 객체에 저장된 모든 값을 삭제) --%>
<%-- 	<%session.invalidate(); %> --%>
	<%-- 
	세션 강제 초기화 시 session 객체 자체가 제거되므로 session 객체가 존재하지 않게 됨
	=> 따라서, session 객체 접근 시 오류 발생하게 된다!
	--%>
<%-- 	<h3>새 세션 여부 : <%=session.isNew() %></h3> <!-- 이 코드 실행 시 오류 발생 --> --%>
	
	<%-- 
	invalidate() 메서드 호출 후에는 새로운 세션 생성 전까지 세션 접근 불가
	=> 다른 페이지로 이동하는 등의 작업 수행을 통해 세션 생성 후 접근 가능
	--%> 
