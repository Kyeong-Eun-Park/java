...
// 데이터베이스 작업 수행을 위해 MemberDAO 클래스의 인스턴스(객체) 생성
MemberDAO dao = new MemberDAO();

// MemberDAO 객체의 insertMember() 메서드를 호출하여 회원 가입 작업 수행
// (메서드에 폼 파라미터로 입력받은 데이터들을 직접 전달할 경우)
// => 파라미터 : 가입 시 입력한 정보(이름, 아이디, 패스워드, 주민번호, E-Mail, 직업, 성별, 취미, 가입동기)
// => 리턴타입 : int(insertCount)
// int insertCount = dao.insertMember(name, id, passwd, jumin, email, job, gender, hobby, motivation);

// DTO 객체를 활용하여 데이터를 저장한 후 DTO 객체를 파라미터로 전달할 경우
// 1) MemberDTO 클래스의 인스턴스 생성
MemberDTO member = new MemberDTO();
// 2) MemberDAO 객체의 Setter 메서드를 호출하여 회원 정보를 멤버변수에 저장
member.setName(name);
member.setId(id);
...
// 3) MemberDAO 객체의 insertMember() 메서드를 호출하여 회원 가입 작업 수행
//=> 파라미터 : 가입 시 입력한 정보(MemberDTO 객체)    리턴타입 : int(insertCount)
