...
<body>
	...
	<%-- c:set 태그를 통해 생성된 str 변수는 EL 을 통해 접근 가능 --%>
	<h3>\${str } = ${str }</h3>	
	<%-- 주의! c:set 태그를 통해 생성된 변수는 기본 자바 문법으로 접근 불가 --%>
<%-- 	<%=str %> --%>
	<%-- 주의2! 자바 문법으로 선언된 변수를 EL 에서 사용하려면 JSTL 에서 변수로 변환해야함 --%>
	<h3>\${num } = ${num }</h3>	<%-- num 값이 출력되지 않음 --%>
	...
	<%
	int num3 = 99;
	// pageContext 객체의 setAttribute() 메서드를 통해 값 저장 시 EL 로 접근 가능함
	pageContext.setAttribute("num3", num3);
	%>
	<h3>\${num3 } = ${num3 }</h3>	<%-- num3 값 출력됨 --%>
