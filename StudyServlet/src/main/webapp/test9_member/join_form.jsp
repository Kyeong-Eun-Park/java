<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%--
< study_jsp3.member 테이블 정의 >
------------------------------------------------
번호(idx) - 정수, PK, 자동증가(auto_increment)
아이디(id) - 문자 8글자, UN, NN
비밀번호(passwd) - 문자 16글자, NN
주민번호(jumin) - 문자 14글자, UN, NN
E-Mail(email) - 문자 50글자, UN, NN
직업(job) - 문자 20글자, NN
성별(gender) - 문자 1글자, NN
취미(hobby) - 문자 10글자, NN
가입동기(motivation) - 문자 2000글자, NN
가입일(hire_date) - 날짜, NN
------------------------------------------------
CREATE TABLE member (
	idx INT PRIMARY KEY AUTO_INCREMENT,
	id VARCHAR(8) UNIQUE NOT NULL,
	passd VARCHAR(16) NOT NULL,
...
<script type="text/javascript">
	var isCorrectId = false; // 아이디 사용 가능 여부를 저장할 변수 선언(true : 가능, false : 불가능)
...
// 			console.log(score);
	...
	*/
	...
	function checkForm(fr) { // 함수 파라미터로 폼 객체 전달받기(document.joinForm = fr)
		...
		if(!isCorrectId) { // isCorrectId == false (아이디 사용 불가)
			alert("아이디 4 ~ 8자리 필수!");
			fr.id.select();
			return false;
...
<body>
	...
	<div align="center">
		<h1>회원가입</h1>
		<!-- 회원 가입 처리 수행 위해 "JoinPro" 서블릿 주소 요청 -> JoinProServlet 클래스 매핑 -->
<!-- 		<form action="JoinPro" name="joinForm" method="post" onsubmit="return checkForm(this)"> -->
		<form action="MemberJoinPro.me" name="joinForm" method="post">
			<table border="1">
				...
				<tr>
					<th>가입동기</th>
					<td>
						<textarea rows="10" cols="60" name="motivation" required="required"></textarea>
					</td>
				</tr>
				<tr>
