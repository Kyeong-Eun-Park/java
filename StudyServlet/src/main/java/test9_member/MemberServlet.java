...
public class MemberServlet extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 자바스크립트를 출력해야하는 경우 null 값을 그대로 사용하기 위해 기본값 null
		// => 자바스크립트 출력이 아닌 일반 포워딩일 경우 ActionForward 객체 생성하여 사용
...
//			dispatcher.forward(request, response);
			...
			// -------------------------------------------------------------------
			// ActionForward 객체를 사용하여 공통 포워딩 작업을 수행하기 위한 설정
			// 1. ActionForward 객체 생성(변수는 이미 선언되어 있음)
			forward = new ActionForward();
			// 2. setPath() 메서드를 호출하여 포워딩 주소를 설정
			forward.setPath("test9_member/member_main.jsp");
			// 3. setRedirect() 메서드를 호출하여 포워딩 방식을 설정
			//    => Dispatch 방식의 포워딩이므로 false 값 설정
			forward.setRedirect(false);
		} else if(command.equals("/MemberJoinForm.me")) {
			// test9_member/join_form.jsp 페이지로 포워딩(Dispatch)
			forward = new ActionForward();
			forward.setPath("test9_member/join_form.jsp");
			forward.setRedirect(false);
		} else if(command.equals("/MemberLoginForm.me")) {
			// test9_member/login_form.jsp 페이지로 포워딩(Dispatch)
			forward = new ActionForward();
			forward.setPath("test9_member/login_form.jsp");
			forward.setRedirect(false);
			...
			if(insertCount > 0) {
				// MemberMain.me 서블릿 주소 요청(리다이렉트 방식 포워딩)
//				response.sendRedirect("MemberMain.me");
				
				forward = new ActionForward();
				forward.setPath("MemberMain.me");
				// 리다이렉트 방식 포워딩일 경우 setRedirect() 메서드에 true 값 전달
				forward.setRedirect(true);
			} else {
				// 주의! 자바스크립트를 사용(출력)할 경우
				// ActionForward 객체를 통한 포워딩을 수행하지 않으므로
				// 자바스크립트 출력 코드만 기술하고 ActionForward 객체는 생성하지 않는다!
			...
			if(id == null || id.equals("") || !id.equals("admin")) {
				...
				forward = new ActionForward();
				forward.setPath("test9_member/member_list.jsp");
		...
		// => 포워딩 방식을 구별해야하고, 포워딩 주소를 사용하여 포워딩 수행해야함
		// => ActionForward 클래스를 정의하여 포워딩 방식과 포워딩 주소 저장하여 사용
		// => 단, ActionForward 객체가 존재할 경우에만 포워딩 작업 수행해야함
		if(forward != null) { // ActionForward 객체가 존재할 경우
			// 포워딩 방식 판별하여 각 방식에 따른 포워딩 작업 수행
			// => ActionForward 객체의 isRedirect() 메서드를 호출하여 판별
			//    (true : Redirect, false : Dispatch)
			if(forward.isRedirect()) { // 리다이렉트 방식
				// 리다이렉트 방식으로 포워딩
				// => ActionForward 객체의 getPath() 메서드 호출하여 포워딩 주소 리턴받아 포워딩
