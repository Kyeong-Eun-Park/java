...
public class Ex1 {
	...
	public static void main(String[] args) {
		...
		badWeek.setMyWeek(BadWeek.WEEK_THURSDAY);
		
		if(badWeek.getMyWeek() == BadWeek.WEEK_THURSDAY) {
			System.out.println("오늘은 목요일 입니다!");
		}
		
		// 요일 정보를 상수로 관리하는 경우의 문제점
		// => 상수 데이터들이 정수일 경우 해당 정수를 전달받은 메서드 (Setter) 호출 시
		//    상수 범위 값이 아닌 다른 값을 전달해도 컴파일에러(문법적오류)가 발생하지 않는다!
...
//		gw.setMyWeek(5);
//		gw.setMyWeek("MONDAY");
		// => 반드시 열거타입명.상수명 으로 지정된 값만 전달해야한다!
...
class GoodWeek {
	...
	public void printWeek() {
		// 저장된 요일(myWeek)을 판별하여 월 ~ 일 까지 출력 => switch ~ case 사용
		// - 주의사항! switch 문에 enum 타입 변수를 전달할 경우
		//   case 문에서 비교값으로 enum 타입 값을 지정할때
		//   열거타입명 없이 상수 바로 사용!
		switch (myWeek) {
